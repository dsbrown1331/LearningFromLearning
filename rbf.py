...
def get_expected_feature_counts_softmax(num_rollouts, fMap, valueFunction, env, discount):
    '''uses a softmax policy, max entropy'''
    features = []
    for i in range(num_rollouts):

        steps, states_visited = sarsa.rollout_softmax(env, valueFunction, render=False)
